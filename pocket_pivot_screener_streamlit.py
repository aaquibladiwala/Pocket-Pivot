...
# Placeholder for NSE holidays (update with 2025 holidays if available)
NSE_HOLIDAYS_2025 = [
...
def get_data_last_updated():
    """Get the most recent last_updated timestamp from stock_data.csv."""
    try:
        df = pd.read_csv('https://raw.githubusercontent.com/aaquibladiwala/Pocket-Pivot/main/stock_data.csv')
        last_updated = df['last_updated'].max()
        if pd.notna(last_updated):
            return pd.to_datetime(last_updated).strftime('%Y-%m-%d %H:%M:%S')
        return "Data not yet initialized"
...
def screen_stocks(symbols, days_back=365, lookback=10, sma_period=50, max_distance_52w_high=0.05, 
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = []
        for symbol in tqdm(symbols, desc="Fetching stocks"):
            futures.append(executor.submit(fetch_stock_data, symbol, days_back))
...
def main():
    ...
    st.markdown("Adjust filters and select conditions to screen for pocket pivot signals in Indian stocks. Results update in real-time. Data is fetched daily and stored in a CSV on GitHub.")

    # Display last updated timestamp
    st.write(f"**Data Last Updated**: {get_data_last_updated()}")

    # Sidebar for filter inputs
    st.sidebar.header("Filter Settings")
    max_symbols = st.sidebar.slider("Max Stocks to Scan", 50, 2000, 500, help="Limit for speed (500 takes ~30-60 secs)")
    ...
    with st.expander("How to Use & Notes"):
        ...
        - **Runtime**: ~30-60 secs for 500 stocks, as data is pre-fetched daily into a CSV on GitHub.
        - **EQUITY_L.csv**: Fetched from GitHub (https://raw.githubusercontent.com/aaquibladiwala/Pocket-Pivot/main/EQUITY_L.csv).
        - **Stock Data**: Fetched daily (Monday–Friday, 6 PM IST) via GitHub Actions and stored in stock_data.csv.
        - **Data Last Updated**: Shows when the stock data CSV was last updated (above or after results).
        - **Filters**:
        ...
        - **Errors**: Check `errors.log` for issues (e.g., CSV access errors).
        - **Deploy**: Hosted on Streamlit Community Cloud for interactive screening.
        """)
